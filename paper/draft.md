...
> 작업 중 초안 (v0.6) — 팀 공통 주제 MEC 하위, 서브주제 "이동성 기반 서비스 마이그레이션 × LLM".
...
> v0.6 변경 요지: 조율을 대체할 수 있는 세 가지 대안을 5-8절에 추가했다(`sim/alternatives.py`). (i) 턴 경계 마이그레이션은 think time이 1 s 이상이면 대부분의 복구를 숨겨 평균 SIT에서 가장 강하지만, think time 0 s에서는 제안보다 나쁘고 현재 턴 종료까지 포워딩 ITL을 낸다. (ii) top-2 전체 복제 헤징은 모든 잡음 수준에서 Pallas의 SIT와 낭비를 악화시켰다. (iii) 코덱 비용을 뺈 이상적 8× KV 압축도 고밀도 시나리오의 GPU/activation 병목을 없애지 못했다. 따라서 현재 결과는 "단일 절대 정책"보다 **턴 상태·think time·경합에 따른 상황별 모드 선택 + 조율** 프레임을 지지한다.
...
**턴 경계 마이그레이션.** 사용자별로 128개 토큰을 연속 생성하고 턴 사이에 4 s를 생각하는 주기를 독립 위상으로 주었다. Turn-Boundary는 무선 핸드오버 후에도 현재 턴이 끝날 때까지 소스에서 생성하며 토큰을 포워딩하고, 생성 종료 후 think interval에 ctHO 복구를 시작한다. 복구 시간이 think time을 넘는 부분만 다음 턴의 SIT로 계산하고, 턴 종료 전까지는 Detour와 같은 포워딩 ITL을 낸다.

| 사용자 | 제안 SIT 평균 / p99 (s) | Turn-Boundary SIT 평균 / p99 (s) | Turn-Boundary ITL (ms) | 완전 은폐율 |
|---:|---:|---:|---:|---:|
| 32 | 0.348 / **0.694** | **0.046** / 1.002 | 72.0 | 92% |
| 64 | 0.354 / **0.853** | **0.050** / 1.114 | 71.9 | 92% |
| 128 | 0.389 / 1.359 | **0.052 / 1.140** | 71.9 | 91% |
| 192 | 0.424 / 1.587 | **0.058 / 1.166** | 71.8 | 91% |

평균 SIT는 모든 밀도에서 Turn-Boundary가 가장 낮다. 그러나 32–64명에서는 느린 복구 소수가 p99를 제안보다 크게 만들고, 기준 ITL 67 ms보다 4.8–5.0 ms 높다. 결과의 think-time 의존성을 192명에서 확인했다.

| Think time (s) | 제안 SIT 평균 / p99 | Turn-Boundary SIT 평균 / p99 | Turn-Boundary ITL (ms) | 완전 은폐율 |
|---:|---:|---:|---:|---:|
| 0 | **0.437 / 1.646** | 1.163 / 2.063 | 73.5 | 0% |
| 1 | 0.433 / 1.631 | **0.279 / 1.348** | 73.7 | 37% |
| 2 | 0.429 / 1.621 | **0.074 / 1.254** | 73.0 | 88% |
| 4 | 0.424 / 1.587 | **0.058 / 1.166** | 71.8 | 91% |
| 8 | 0.417 / 1.551 | **0.043 / 1.108** | 70.8 | 93% |

think time이 0 s면 Turn-Boundary는 전체 복구가 다음 턴을 막아 제안에 지고, 1 s부터는 평균·p99에서 앞선다. 이는 SIT-only 서사를 수정할 정도로 강하지만, 실제 턴 길이·think-time 분포와 소스 GPU 상주 비용을 뺈 이상화된 상한이다. 결과가 지지하는 구조는 `생성 중→Turn-Boundary`, `다음 턴이 즉시 도착→조율형`, `짧은 문맥→반응형`의 상황별 모드 선택이다.
...
- **턴 모델.** Turn-Boundary는 고정 128-token output과 고정 think time을 독립 위상으로 배치한 모델이다. ShareGPT/LMSYS의 실측 턴 길이·사용자 응답 간격 분포, 즉시 연속 턴, 툴 호출·스트리밍 중단을 반영해야 한다. 또한 턴 종료까지 소스 GPU에 상주하는 세션의 연산·VRAM 비용은 빠져 있다.
...
추가 비교에서 top-2 전체 복제 헤징과 이상적 8× KV 압축은 herding을 해소하지 못했다. 반면 Turn-Boundary는 think time이 1 s 이상일 때 SIT에서 제안을 앞섰으므로, 최종 시스템은 단일 정책이 아니라 턴 상태·예상 think time·문맥 길이·자원 경합에 따라 Turn-Boundary, 조율형 선제 준비, 반응형을 선택해야 한다.
