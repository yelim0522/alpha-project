...
> 작업 중 초안 (v0.8) — 팀 공통 주제 MEC 하위, 서브주제 "이동성 기반 서비스 마이그레이션 × LLM".
...
엣지 서버 간 KV 캐시 이전과 다중 이전의 공동 최적화는 이미 연구되었다. EdgeFlow [edgeflow]는 분산 엣지 서버의 과부하·이탈 상황에서 이전 대상 서버와 KV 직접 전송·중간 활성값 기반 재계산의 비율을 함께 결정한다. 본 연구의 질문은 이동 사용자의 기지국 전환 시각이 주어졌을 때, 같은 목표 서버에 몰리는 세션들의 준비를 **언제 시작할지**이다.
...
EdgeFlow는 KV 이전 메커니즘과 서버 배정의 선행 연구이며, Pallas는 이동 사용자 선제 이전의 직접 비교군이다. 아래 표는 각 연구의 결정 변수를 구분한다.
...
| 본 연구 | 복수 사용자의 근접한 전환 예측 | 같은 목표 기지국의 준비 시점·자원 조율 | 경합 상황의 사용자 중단 시간 |
...
- **턴 모델.** Turn-Boundary는 고정 128-token output과 고정 think time을 독립 위상으로 배치한 모델이다. 이전 중 타겟 GPU·백홀 점유는 반영했지만, 복구 지연이 실제 다음 턴의 토큰 생성 시각을 늦추는 효과는 공통 궤적·토큰 트레이스 때문에 반영하지 못한다. 턴 종료까지 소스 GPU에 상주하는 세션의 연산·VRAM 비용도 빠져 있다. 실제 턴 길이·응답 간격 분포, 즉시 연속 턴, 툴 호출·스트리밍 중단을 반영해야 한다.
