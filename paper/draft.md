...
- **선제형이 반응형보다 나쁜 두 모서리가 있다.** (i) 문맥이 0.5K 이하로 짧으면 재계산이 0.3 s 안에 끝나므로, 준비된 캐시를 활성화하는 고정 비용($T_0^{p}$=236 ms, ctHO의 $T_0$=75 ms보다 크다)이 이득을 넘어선다. 이 모서리는 백홀이 빨라질수록 넓어진다(3 Gbps에서 1.25배). (ii) 문맥이 길고(8K) 사용자가 많거나(192명) 백홀이 좁으면(100 Mbps) herding으로 준비가 데드라인 안에 끝나지 못한다. 56셀 중 8셀에서 Pallas의 평균 SIT가 ctHO보다 크다.
- **꼬리에서는 훨씬 넓게 진다.** p99 기준으로는 56셀 중 25셀에서 Pallas가 ctHO보다 나쁘다: 4.5K 이상 문맥의 128명 이상, 8K 문맥 전 구간, 100 Mbps 전 구간, 잡음이 있는 128명 이상 구간이다. Pallas는 평균을 낮추는 대신 동기화된 트리거로 꼬리를 키우는 정책이다. 평균만 보면 Pallas가 거의 항상 이기지만, worst-user 기준으로는 지도의 절반에서 반응형에 진다.
- **조율은 herding 모서리를 없애지만 활성화 모서리는 공유한다.** 제안은 56셀 모두에서 Pallas 이하의 평균 SIT를 내고, p99에서는 ctHO에 지는 셀이 없다. Pallas가 지는 (ii) 모서리에서 제안은 ctHO 대비 0.32배(8K·192명)와 0.23배(100 Mbps·8K)이다. 반면 (i) 모서리에서는 제안도 같은 활성화 비용을 내므로 ctHO에 3–20% 뒤진다. 이는 조율의 한계가 아니라 "짧은 문맥은 그냥 재계산하라"는 모드 선택의 문제이며, 무경합 시 손익분기 문맥 길이 $L^{\dagger}\approx v_1(T_0^{p}-T_0)$(Qwen3-32B에서 약 320 토큰) 아래이면 준비를 생략하고 반응형 재계산을 택하는 규칙으로 닫힌다(6절).
...
이 모델에서 think time이 0–1 s면 Turn-Boundary는 평균·p99 모두 제안에 진다. 2–8 s에서는 평균은 앞서지만 p99는 계속 크게 뒤진다. 이는 평균 SIT만으로 정책의 우월성을 판단하기 어렵다는 뜻이다. 실제 턴 길이·생각하는 시간 분포, 소스 GPU 상주 비용, 정책별 대화 시각의 변화를 반영하기 전에는 최종 우월성으로 주장하지 않는다.
...
5-8절은 동일한 토큰 트레이스를 재생하므로 이전 중단이 다음 턴의 시각에 영향을 주지 않았다. 이를 검증하기 위해 이동 경로와 사용자별 작업 표본 순서를 공유하되, 정책별 복구 대기와 토큰 전달 지연에 따라 실제 생성·다음 질문 시각이 늦어지는 대화 시계를 추가했다. Azure 공개 대화 요청 자료 [azuretrace]의 응답 길이 25,202개 표본을 사용하고, 생각 시간은 실측이 아닌 평균 4 s의 혼합 분포(25% 즉시 질문, 나머지 로그정규)로 두었다. 시드별 원자료·설정·코드 해시와 상세 한계는 `paper/turn_boundary_validation.md` 및 `sim/turn_validation_v09_*`에 기록했다.

192명·600 s·시드 3개에서 Azure 길이와 혼합 생각 시간을 적용했을 때 **완료 응답별 실제 생성 중단 시간**의 평균/p99는 조율형 0.092/1.140 s, 턴 경계 0.093/1.959 s였다. 이 지표는 이전 사건별 SIT와 분모가 다르므로 5-8절의 수치와 직접 비교하지 않는다. 응답 완료까지의 추가 시간(중단+전달 지연)의 p99는 각각 1.140/9.296 s였고, 턴 경계의 완료 응답 수는 11.02% 적었다. 초기 부분 관측 응답은 완료 응답 통계에서 제외하며, 종료·재진입으로 미완료된 응답과 실제 관측 대기 합계도 별도로 보고했다.

고정 길이·합성 가변 길이·실측 길이·즉시 후속 질문·생성 처리량 제한·시간 간격 변경을 포함한 7조건 × 3시드 모두에서 턴 경계의 응답 대기 p99가 조율형보다 컸다. 기본 0.5 s 대신 0.25 s 주기를 쓴 주요 조건에서도 p99는 1.149/1.971 s, 완료 응답 감소율은 10.88%로 방향이 유지됐다. 서버당 생성 예산을 240 token/s로 제한한 미보정 민감도에서는 두 방식 모두 느려지고 완료 응답 차이는 3.21%로 줄었다.

원격 서빙 동안 남은 KV의 적분량도 측정했다. 주요 조건에서 턴 경계는 18,748 GB·s로, 전체 서버 합산 평균 약 31.2 GB가 무선 서빙 서버와 다른 서버에 남아 있었다. 이 값은 전체 VRAM 또는 추가 복제 메모리가 아니며 위치가 남아 있는 상태의 양이다. 따라서 평균 중단 시간만으로 턴 경계 방식의 우월성을 주장하기 어렵다. 다만 이 실험은 실측 생각 시간, 실제 대화 순서, 전체 GPU 혼합 실행이나 메모리 승인·퇴거까지 재현하지 않는다.
...
- **활성화 고정 비용과 모드 선택.** 제안은 Pallas의 준비-활성화 경로($T_0^{p}$=236 ms)를 그대로 쓰므로, 문맥이 수백 토큰 이하로 짧아 재계산이 그보다 빨리 끝나는 구간에서는 두 선제 정책 모두 반응형에 3–20% 뒤진다(5-7절). 손익분기 $L^{\dagger}\approx v_1(T_0^{p}-T_0)$ 아래에서 준비를 생략하는 규칙은 계획기에 한 줄로 들어가지만, 경합 시 $L^{\dagger}$가 움직이므로 계획된 점유 위에서 재계산 완료 시각과 비교하는 형태가 맞다. 또한 $T_0^{p}$ 자체는 Pallas 측정값에 맞춘 상수이며, 준비된 KV를 붙이는 엔진 구현에 따라 크게 달라질 수 있다.
...
추가 비교에서 top-2 전체 복제 헤징과 이상적 8× KV 압축은 herding을 해소하지 못했다. Turn-Boundary는 생각하는 시간을 이용해 평균 SIT를 낮출 수 있지만, 진행 중인 이전 작업의 공유 자원 경합을 반영하면 고밀도 p99는 제안보다 계속 크다. 따라서 턴 상태·예상 think time·문맥 길이·자원 경합에 따른 모드 선택은 유망한 후속 방향이지만, 실제 대화 간격과 소스 상주 비용을 반영하기 전에는 최종 시스템의 우월성으로 주장하지 않는다.
